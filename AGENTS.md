<!-- TeamShell Protocol Start v0.1.0 -->
<!-- TeamShell Protocol -->
...
tsh send <编号> --key "\x03"       → 帮他按 Ctrl+C
```
...
**入职第一件事**：
1. 读取你的工号：`echo $TEAMSH_TAB_ID`
2. 读取你的名字：`echo $TEAMSH_NAME`（老板起的，直接用）
3. 如果 `TEAMSH_NAME` 为空，自己想一个名字
4. 用 `tsh name $TEAMSH_TAB_ID "你的名字"` 确认（这样同事的界面上才能看到你的名字）
5. 认识同事：`tsh list`
