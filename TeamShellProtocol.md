...
1. **身份标识存在且合法**
   - 环境变量 `TEAMSH_TAB_ID`（你的工号）存在且为正整数
   - 环境变量 `TEAMSH_NAME`（你的名字）存在且非空
   - 环境变量 `TEAMSH_PLATFORM`（你的平台）存在且非空
...
> 本规则为快速参考。如与 3.1-3.7 的详细定义冲突，以详细定义为准。
...
你的工号是：`$TEAMSH_TAB_ID`（Windows 用 `$env:TEAMSH_TAB_ID`，macOS/Linux 用 `echo $TEAMSH_TAB_ID`）
你的名字是：`$TEAMSH_NAME`
